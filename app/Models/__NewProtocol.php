...
class Protocol extends Model implements Auditable
{
    ...
    public function storeProtocolAPIDetail($protocol, $request)
    {
        ...
        // Ensure $childInfoData is an array, even if it's null or a string
        $childInfoData = is_string($request->ApiID) ? json_decode($request->ApiID, true) : ($request->ApiID ?? []);

        // Ensure $childInfoData is an array before iterating
        if (is_array($childInfoData)) {
            foreach ($childInfoData as $key => $value) {
    ...
    public function storeProtocolProductDetails($protocol, $request): Protocol
    {
        ...
        $childInfoData = is_string($request->SkuID) ? json_decode($request->SkuID, true) : ($request->SkuID ?? []);

        if (is_array($childInfoData)) {

            foreach ($childInfoData as $key => $sku) {
    ...
    public function storeProtocolSkuContainerType($protocol, $request): Protocol
    {
        ...
        if (is_array($childInfoData)) {
            ...
            foreach ($childInfoData as $index => $sku) {
    ...
    public function storeProtocolPackagingProfile($protocol, $request): Protocol
    {
        ...
        $childInfoData = is_string($request->all) ? json_decode($request->all, true) : ($request->all ?? []);
