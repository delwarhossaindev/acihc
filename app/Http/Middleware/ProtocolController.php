...
class ProtocolController extends Controller
{
        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($data)
                ...
                ->addColumn('protocolNo', function ($protocol) {
                    ...
                    return 'Protocol No:STB/PROT/' . sprintf('%04d', $protocol->ProtocolID) . '; Version: ' . $versionCount;
                })
    ...
    public function reasonStore(Protocol $protocol, Request $request)
    {
        try {
            ...
            // Save new protocol data
            $protocolNewData->save();

            // Call additional protocol storage methods
           $result =  $protocol->storeProtocolAPIDetail($protocolNewData, $protocolPreviousData);

            dd($result);
            $protocol->storeProtocolProductDetails($protocolNewData, $protocolPreviousData);
            $protocol->storeProtocolSkuContainerType($protocolNewData, $protocolPreviousData);
            $protocol->storeProtocolPackagingProfile($protocolNewData, $protocolPreviousData);
            $protocol->storeProtocolStabilityStudy($protocolNewData, $protocolPreviousData);

            // Handle batch design, placebo design, and chamber design
            ...
            \Log::error('Error in reasonStore method: ' . $e->getMessage());
