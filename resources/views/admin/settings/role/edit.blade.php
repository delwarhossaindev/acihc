...
@section('content')
    ...
    //  $role_permission = $user->getPermissionsViaRoles()->pluck('id')->toArray();

     // dd( $user->hasPermission('DashboardController@index'),$permissions = $user->allPermissions()->pluck('name')->toArray(),$user->can('DashboardController@index'));
     @endphp
            ...
            @endif
                <div class="form-check form-switch mb-2 form-inline">
                    <input
                        ...
                        name="permissions[]"
                        value="{{ $permission->id }}"
                        {{ !empty($role_permission[$permission->id]) ? 'checked' : '' }}
                    >
